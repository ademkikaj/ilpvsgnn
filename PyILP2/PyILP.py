...
def aleph(aleph_bk, pos_fold_ex, neg_fold_ex, settings, q=None):
    ...
    if type(settings) == list:
        ...
        file_1.write("\n:-end_in_neg.\n")
        file_1.close()
    #proc = mp.Process(target=learn_theory_aleph, args=(temp_file,))
    #proc.start()
    #proc.join()
...
def aleph_learn(file="BK.pl", test_size=0.33, positive_example="pos_example.f",
    ...
    # q = mp.Queue()
    # proc = mp.Process(target=aleph, args=(file, positive_example_ids_training, negative_example_ids_training, settings,q))
    # proc.start()
    # proc.join()
    # H_2 = q.get()
    ...
    if test_size != 0:
        ...
        #print(H_2)
        #print("+----------+ Learning +----------+")
        test = evaluate_theory_prolog(H_2, file, positive_example_ids_training, 
        negative_example_ids_training)
        print("Accuracy: ",test.accuracy)
        print("hypothesis: ",H_2)
        os.remove("theory.txt")
        return H_2,test.accuracy
        return Aleph(H_2, test.accuracy, test.precision, test.sensitivity, test.specificity, test.fscore,
                     aleph_learn_time)
