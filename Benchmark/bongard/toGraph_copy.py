...
class toGraph:
    ...
    def collect_problemId_objects(self,index):
        ...
        graph_id = index
        triangles = self.kb["triangle"][self.kb["triangle"][self.problem_key] == graph_id]
        squares = self.kb["square"][self.kb["square"][self.problem_key] == graph_id]
        circles = self.kb["circle"][self.kb["circle"][self.problem_key] == graph_id]
        ins = self.kb["in"][self.kb["in"][self.problem_key] == graph_id]
    ...
    def node_only(self):
        ...
        for graph_index in range(self.num_of_graphs):
            ...
            triangles,squares,circles,ins = self.collect_problemId_objects(graph_id)
            dataframes = [triangles,squares,circles,ins]
            ...
            # create node feature array
            num_nodes = len(objectIds) + len(ins)
    ...
    def node_edge(self):
        ...
        for graph_index in range(self.num_of_graphs):
            ...
            # create node features
            num_of_node_features = len(node_predicates)
    ...
    def edge_based(self):
        ...
        for graph_index in range(self.num_of_graphs):
            ...
            num_node_features = 3 + 1

            # external and internal predicates
            num_edge_features = 2

            # create the node features 
            node_features = np.zeros((len(objectIds) + 3, num_node_features))

            node_features[0][0] = 1
            node_features[1][1] = 1
            node_features[2][2] = 1
            for i in range(3,len(objectIds) + 3):
                node_features[i][3] = 1

            # create the edges
            object_indices = {"square": 0, "circle": 1, "triangle": 2}
            edge_indices = [[],[]]
            edge_features = []
            ...
            for index,row in ins.iterrows():
                obj1 = row["objectId1"]
                obj2 = row["objectId2"]
                index1 = np.where(objectIds == obj1)[0][0] + 3
                index2 = np.where(objectIds == obj2)[0][0] + 3
                edge_indices[0].append(index1)
                edge_indices[1].append(index2)
    ...
    def Klog(self):
        ...
        for graph_index in range(self.num_of_graphs):
            ...
            # create node features
            node_features = np.zeros((len(objectIds) + len(ins), len(dataframes)))
