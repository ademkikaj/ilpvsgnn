...
# create the movie encoder
movies = pd.read_csv(os.path.join(relational_path, "movie.csv"))["movie"].unique()
print("Amount of different movies: ", len(movies))
movie_encoder, movie_decoder = {},{}
for i,movie in enumerate(movies):
    movie_encoder[movie] = torch.zeros(len(movies))
    movie_encoder[movie][i] = 1
    movie_decoder[tuple(movie_encoder[movie].numpy())] = movie

# create the name encoder
names_movies = list(pd.read_csv(os.path.join(relational_path, "movie.csv"))["person"].unique())
names_genre = list(pd.read_csv(os.path.join(relational_path, "genre.csv"))["person"].unique())
names_gender = list(pd.read_csv(os.path.join(relational_path,"gender.csv"))["person"].unique())
names_director = list(pd.read_csv(os.path.join(relational_path,"director.csv"))["name"].unique())
names_actor = list(pd.read_csv(os.path.join(relational_path,"actor.csv"))["name"].unique())
name_encoder,name_decoder = {},{}
all_names = names_movies + names_genre + names_gender + names_director + names_actor
names = list(set(all_names))

for i,name in enumerate(names):
    name_encoder[name] = torch.zeros(len(names))
    name_encoder[name][i] = 1
    name_decoder[tuple(name_encoder[name].numpy())] = name

# create genre encoder
genres = pd.read_csv(os.path.join(relational_path, "genre.csv"))["genre"].unique()
genre_encoder, genre_decoder = {},{}
for i,genre in enumerate(genres):
    genre_encoder[genre] = torch.zeros(len(genres))
    genre_encoder[genre][i] = 1
    genre_decoder[tuple(genre_encoder[genre].numpy())] = genre
...
import sys
from Benchmark.imdb.toGraph import toGraph
from Benchmark.imdb.toLogic import toLogic
from Benchmark.toILP import toILP

graph_converter = toGraph(relational_path=relational_path_train,dataset_name=dataset_name,dataset_problem_key=problem_id,target=target, movie_encoder=movie_encoder, name_encoder=name_encoder, genre_encoder=genre_encoder)
graph_converter_test = toGraph(relational_path=relational_path_test,dataset_name=dataset_name,dataset_problem_key=problem_id,target=target, movie_encoder=movie_encoder, name_encoder=name_encoder, genre_encoder=genre_encoder)

logic_converter = toLogic(relational_path=relational_path_train,dataset_name=dataset_name,movie_decoder=movie_decoder,name_decoder=name_decoder,genre_decoder=genre_decoder)
logic_converter_test = toLogic(relational_path=relational_path_test,dataset_name=dataset_name,movie_decoder=movie_decoder,name_decoder=name_decoder,genre_decoder=genre_decoder)
...
for repr in representations:
    ...
    if repr == "node_only":
        ...
        tilde_settings = [
            ...
            "warmode(person(+id,+-node_id,+-name)).\n",
            "warmode(movie(+id,+-node_id,+-movie)).\n",
            "warmode(director(+id,+-node_id,+-node_id)).\n",
            "warmode(actor(+id,+-node_id,+-node_id)).\n",
